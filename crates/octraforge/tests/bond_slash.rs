...
fn gov_slash_burns_stake_and_pays_bounty() {
    ...
    let bounty_amt = slashed
        ...
        .and_then(serde_json::Value::as_u64)
        .unwrap();
    assert_eq!(burn_amt + bounty_amt, octra_mock_rpc::MIN_ENDPOINT_STAKE);
    assert_eq!(
        burn_amt,
        octra_mock_rpc::MIN_ENDPOINT_STAKE * octra_mock_rpc::SLASH_BURN_BPS / 10_000
    );
...
fn slashed_operator_cannot_re_register() {
    ...
    ctx.prank(&addr);
    let err = ctx
        .call_bond_endpoint(octra_mock_rpc::MIN_ENDPOINT_STAKE)
