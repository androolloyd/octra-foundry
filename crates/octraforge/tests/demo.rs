...
octra_test!(full_lifecycle_endpoint_tailnet_session_claim, |forge| {
    ...
    // 7. Client confirms matching bytes; this is what actually applies
...
octra_test!(expect_revert_on_bad_settle, |forge| {
    ...
    // Operator claims first (no settlement applies yet).
    forge.prank(VALIDATOR);
    forge.call_settle_claim(sid, 2).expect("settle_claim");

    // Client confirms with the same bytes — at confirm time the AML
    // computes 2 * 100 = 200 OU > 100 OU deposit and reverts.
    forge.prank(CLIENT);
    forge.expect_revert("claim exceeds escrow");
    let res = forge.call_settle_confirm(sid, 2);
...
octra_test!(wrong_revert_substring_surfaces_diff, |forge| {
    ...
    let res = forge.call_settle_claim(999_999, 1);
    match res {
