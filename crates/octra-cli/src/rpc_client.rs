...
//!
//! The CLI needs to make arbitrary-method calls (`cast rpc`), not just a
//! typed surface, so we dispatch raw `(method, params)` pairs and return
//! raw `serde_json::Value`. The same code path also drives integration
//! tests, which run against the in-process `octra_mock_rpc` without any
//! network: `Endpoint::InProcess` carries an `AppState` we can poke
