...
/// Bare async JSON-RPC client used in place of the OctraVPN-specific
/// `octravpn_core::rpc::RpcClient`. The CLI's own `rpc_client::call`
/// runs a fresh blocking tokio runtime per call (so it can stay a sync
/// API for subcommands), which would panic inside a `#[tokio::test]`.
...
async fn rpc_contract_call(
    ...
    rpc_call(url, "contract_call", json!([program, method, params])).await
}
...
async fn http_anvil_matches_inprocess_view() {
    ...
    // Plain HTTP JSON-RPC. We avoid octravpn-core here so the test stays
    // confined to the foundry-side toolchain (the CLI, the mock, and a
    // bare reqwest client).
...
async fn http_anvil_register_then_list_matches_forge() {
    ...
    let url = http_url(18302);

    let val = "octHTTPV0000000000000000000000000000VALI";

    // Seed validator status + operator stake via the test helpers.
    rpc_call(&url, "octra_test_grantValidator", json!([val]))
        .await
        .unwrap();
    rpc_call(&url, "octra_test_bondEndpoint", json!([val]))
        .await
        .unwrap();
